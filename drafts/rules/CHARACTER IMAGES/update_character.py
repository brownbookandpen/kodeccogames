...
transparent PNG, saves them into rules/character/images/0N.png, rewrites the
stack section of rules/character/character.html (creating the page from a
template on first run), and updates the "CHARACTER" card thumbnail on the
...
# CHARACTER IMAGES -> rules -> drafts -> kodeccogames (repo root)
...
<div class="post-header">
  ...
  <h1>CHARACTER</h1>
  <div class="subtag">[PUBLIC ROLE]</div>
</div>
...
def ensure_page():
    ...
    html = PAGE_TEMPLATE.replace("CHARACTER", TITLE).replace("[PUBLIC ROLE]", SUBTAG)
