...
transparent PNG, saves them into rules/mission/images/0N.png, rewrites the
stack section of rules/mission/mission.html (creating the page from a
template on first run), and updates the "MISSION" card thumbnail on the
...
Run via UPDATE_MISSION.bat (double-click) or:
    python update_mission.py
"""
import re
import sys
...
# MISSION IMAGES -> rules -> drafts -> kodeccogames (repo root)
...
<div class="post-header">
  ...
  <h1>MISSION</h1>
  <div class="subtag">[OBJECTIVES]</div>
</div>
...
def ensure_page():
    ...
    html = PAGE_TEMPLATE.replace("MISSION", TITLE).replace("[OBJECTIVES]", SUBTAG)
