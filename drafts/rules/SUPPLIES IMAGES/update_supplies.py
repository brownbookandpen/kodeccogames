...
transparent PNG, saves them into rules/supplies/images/0N.png, rewrites the
stack section of rules/supplies/supplies.html (creating the page from a
template on first run), and updates the "SUPPLIES" card thumbnail on the
...
# SUPPLIES IMAGES -> rules -> drafts -> kodeccogames (repo root)
...
def ensure_page():
    ...
    html = PAGE_TEMPLATE.replace("SUPPLIES", TITLE).replace("[LOADOUT]", SUBTAG)
